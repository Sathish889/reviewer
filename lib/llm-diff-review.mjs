...
async function reviewSlice(engine, secs, depth, tag) {
  ...
  // No severity rewriting here. An earlier version capped anything whose text matched /\bstyle\b/ at
  // the profile's severity, which meant a real high finding that merely used the word "style" was
  // silently downgraded — a heuristic that can weaken a genuine finding is worse than the noise it
  // removes. The prompt already tells the style mandate which severity to use, and the deterministic
  // checks below are capped where they are produced.
