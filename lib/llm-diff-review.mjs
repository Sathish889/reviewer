...
const BUDGETS = {
  ...
  thorough: { lenses: ['correctness', 'security', 'structure', 'qa'],
...
const COMPOSITES = {
  code: { pair: ['correctness', 'structure'], title: 'STAFF ENGINEER reviewing CORRECTNESS, REGRESSIONS and STRUCTURE' },
...
const BASE_LENS_NAMES = ['correctness', 'security', 'structure', 'qa'];
const FULL_LENS = {
...
if (process.env.LLM_REVIEW_REPORT) {
  try {
    fs.writeFileSync(process.env.LLM_REVIEW_REPORT, JSON.stringify({
      ...
      counts: { high: findings.filter((f) => f.sev === 0).length, medium: findings.filter((f) => f.sev === 1).length, low: findings.filter((f) => f.sev === 2).length },
...
const worst = Math.min(...findings.map((f) => f.sev), 3);
if (worst <= threshold) {
