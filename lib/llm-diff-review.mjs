...
0) SAME-CLASS SWEEP — do this for EVERY defect you find, and do it before you write the finding.
...
3) PAIRED FILES ARE ONE UNIT. Where a language or framework splits a component across files that must
...
4) LAYERING & BOUNDARIES. Which layer does the changed code sit in (UI, API/controller, domain/service,
...
5) STRUCTURAL QUALITY of the changed code. Duplicated logic that already exists elsewhere in the repo
...
6) DEAD & LEFTOVER CODE from this change: an empty or no-op function; a branch that can never be taken; a
...
7) OPERABILITY. A new failure path with no log or metric; a log that prints a secret or a whole object; a
...
const conventions = projectConventions(rr);
...
const contextHints = `${rootInventory(rr)}${convBlock}${missBlock}
...
const keyFor = (sec, engine) => cacheKey([
  'v1', sec.text, engine.lens, `${engine.provider.name}:${engine.provider.model}`, ...CACHE_SCOPE,
]);
  ...
  if (depth === 0) {
    for (const sec of secs) {
      ...
      const mine = parsed.filter((f) => f.file === sec.file)
        .map((f) => ({ file: f.file, line: f.line, issue: f.issue, sev: f.sev }));
    ...
    for (const sec of chunks[ci]) {
      ...
      if (hit) {
        ...
        for (const f of hit.findings || []) {
          cachedFindings.push({ file: f.file, line: f.line, issue: f.issue, sev: f.sev, lenses: new Set([engine.lens]), fromCache: true });
        }
...
await pool(runJobs.map((j) => () => reviewSlice(j.engine, j.sections, 0, `${j.engine.lens}@${j.engine.provider.name}${chunks.length > 1 ? ` c${j.ci + 1}` : ''}`)), CONCURRENCY);
...
if (process.env.LLM_REVIEW_REPORT) {
  try {
    fs.writeFileSync(process.env.LLM_REVIEW_REPORT, JSON.stringify({
      ...
      deterministic: DET_HITS.map((d) => ({ file: d.file, line: d.line, check: d.id, severity: SEV_NAME[d.sev] })),
