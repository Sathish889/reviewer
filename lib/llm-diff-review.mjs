...
Style findings are the least severe thing in this review: report them at __STYLE_SEVERITY__ and never
inflate them. A real bug always outranks a formatting point.`,
};
...
function render(f) {
  ...
  return `- ${f.file}:${f.line} :: ${f.issue}${agree}${contested} (${SEV_NAME[f.sev]})`;
}
...
async function reviewSlice(engine, secs, depth, tag) {
  ...
  // The style mandate rides inside a bigger pass, so its findings arrive mixed in with real ones and
  // carry whatever severity the model chose. Style is capped at the profile's severity by definition —
  // otherwise a formatting point could block a commit the author never agreed to block on.
  for (const f of parsed) {
    if (!/\bstyle\b/i.test(f.issue) && !/^\s*(line is \d+ chars|trailing whitespace|indented with)/i.test(f.issue)) continue;
...
if (worst <= threshold) {
  const n = findings.filter((f) => f.sev <= threshold).length +
    (styleHits.length && styleSevIdx <= threshold ? styleHits.length : 0);
