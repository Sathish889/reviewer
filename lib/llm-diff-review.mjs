...
const BUILTIN_PROVIDERS = {
  claude: {
    bin: 'claude', defaultModel: 'opus',                // key: ANTHROPIC_API_KEY — Opus is the strongest tier; used for the deep security review
...
4) SECURITY TESTING — a first-class pass (SAST + risk + posture + runtime test-plan), applied to EVERY changed function, endpoint, and data flow. Treat all new/changed input as UNTRUSTED until it is validated AND encoded at the sink; trace each source→sink path across every trust boundary the change crosses (client↔server, service↔service, app↔DB, app↔third-party).
   • SAST (static — report each as a finding): injection (SQL / NoSQL / OS-command / LDAP / XPath / template-SSTI / CRLF / log); XSS (reflected, stored, DOM); SSRF; path traversal / zip-slip / LFI-RFI; insecure deserialization; XXE; authN (missing/weak auth, credential & password storage — must be bcrypt/argon2/scrypt, timing-unsafe comparisons, JWT alg:none / weak secret / missing exp-aud-iss); authZ — HIGHEST PRIORITY (missing checks, IDOR/BOLA object-level, BFLA function-level, privilege escalation, mass-assignment / over-posting, multi-tenant isolation); crypto misuse (MD5/SHA1/DES/RC4/ECB, hardcoded or static keys/IV/salt, insecure randomness used for security); hardcoded secrets/keys/tokens; sensitive-data exposure & over-logging (PII/PCI/PHI, secrets or tokens in logs, verbose errors/stack traces to the client); CSRF; open redirect; permissive CORS (wildcard origin + credentials); insecure cookies (missing HttpOnly/Secure/SameSite); TOCTOU / races on a security decision; ReDoS and unbounded/DoS (missing rate-limit on auth or expensive endpoints, decompression bombs); language traps (JS prototype pollution, eval/Function, unsafe child_process; Python shell=True / yaml.load; unsafe reflection); and IaC/config issues when such files change (Dockerfile running as root or baking secrets, k8s privileged/hostPath, open security groups, CI secret leakage or unpinned actions).
   • DEPENDENCY / VULN SCAN: flag new or updated dependencies that are unpinned, abandoned, typosquat-looking, run install/lifecycle scripts, or that you recognize as historically vulnerable. You cannot authoritatively enumerate CVEs from static text — recommend the authoritative scan and mark it [RUNTIME]: osv-scanner / npm audit / pip-audit / trivy fs.
   • THREAT MODEL (STRIDE): for the component(s) touched, reason briefly about Spoofing, Tampering, Repudiation, Information disclosure, Denial of service, and Elevation of privilege that this change newly introduces or worsens; a materially raised threat with no mitigating control is a finding.
   • DAST / IAST / PENTEST (runtime-only — you are reviewing static code and CANNOT execute these): for each affected endpoint/flow, EMIT a runnable attack test-plan as a finding prefixed [RUNTIME] — the concrete malicious input/payload, the abuse/misuse case, and the tool that would confirm it (OWASP ZAP / Burp / nuclei for DAST, a runtime agent for IAST, a manual step for a business-logic pentest). Never silently drop a security issue you suspect but cannot prove statically — report it as [RUNTIME] with the exact test that would settle it.
   • RISK & POSTURE: rate each security finding by real exploitability AND impact; anything that reaches a real consumer (RCE, auth bypass, injection, secret exposure, account takeover, data loss) is high. If the code moves money → note PCI-DSS exposure; if it handles EU personal data → note GDPR exposure.
...
- security (SAST): every class in the SECURITY TESTING pass above — injection, XSS, SSRF, path traversal, (de)serialization/XXE, auth & authorization incl. IDOR/BOLA, crypto misuse, hardcoded secrets, sensitive-data exposure, CSRF, CORS, ReDoS/DoS — each tagged with its CWE + OWASP category
...
const reviewPromptFor = (diffText) => `You are a meticulous STAFF-level engineer doing an EXHAUSTIVE code review of a git diff for the repository at ${repoRoot} (${label}). GOAL: catch EVERY real problem this change introduces or exposes — correctness, regressions, security, contract/integration breaks, and unmet requirements — so that NO downstream reviewer (another AI reviewer, a bot, a human, or a security scanner) is left with a legitimate issue you missed. Missing a real problem is the PRIMARY failure; noisy nits are the secondary one.
${contextHints}
...
- RUNTIME-only issues (DAST / IAST / pentest / dependency-CVE scan): prefix <issue> with [RUNTIME] and include the exact payload/test/tool that would confirm it — never drop a suspected security issue just because static review can't prove it.
